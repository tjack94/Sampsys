	...
	render() {
		const nextButton =
			this.state.questionType === '' ? null : (
				<Link to= {`/create-survey/step4/${this.state.questionType}`}><button onClick = {()=> this.props.setQuestionType(this.state.questionType)} > Next</button></Link>
			);
		return (
			<div>
				<h2>Select Question Type</h2>
				<select onChange={(e) => this.handleChange(e.target.value)} name="Question-type">
